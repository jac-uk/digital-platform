...
describe(COLLECTION_NAME, () => {
  ...
  context('Read', () => {
    ...
    it('allow JAC admin with permission to list qualifying test responses', async () => {
      const db = await setup(
        ...
        getEnabledPermissions([PERMISSIONS.qualifyingTestResponses.permissions.canReadQualifyingTestResponses.value])
      );
      await setupAdmin(db, mockData);
  ...
  context('Update', () => {
    ...
    it('allow authenticated user with verified @judicialappointments.digital email and permission to update a qualifying test', async () => {
      const db = await setup(
        { ...mockVerifiedJACDigitalUser, ...mockRoleId },
        getEnabledPermissions([PERMISSIONS.qualifyingTestResponses.permissions.canUpdateQualifyingTestResponses.value])
      );
      await setupAdmin(db, mockData);
