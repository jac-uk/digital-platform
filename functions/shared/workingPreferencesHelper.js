...
const workerPreferences = ['locationPreferences', 'jurisdictionPreferences', 'additionalWorkingPreferences'];
function isWorkingPreferenceColumn(column) {
  const prefixes = workerPreferences.map(preference => `${preference}.`);
  return prefixes.some(prefix => column.startsWith(prefix));
}

/**
 * 
 * @param {Array} answers [{ answer: String, group: String, rank: Number }]
...
function formatAnswers(answers) {
  ...
  for (const answer of answers) {
    ...
    if (answer.rank) {
      chunks.push(`${answer.rank}: `);
    }
    
    if (answer.group) {
      chunks.push(`${answer.group} - `);
    }

    if (answer.answer) {
      chunks.push(answer.answer);
    }

    formattedAnswers.push(chunks.join(''));
  }
