...
module.exports = (firebase, db) => {
  ...
  function getHtmlEligibilityIssues(exercise, applicationRecords) {
    ...
    writer.addRaw(`
      <div style="font-weight: 700; font-size: 11pt; text-align: center;">
        <p style="font-size: 10pt;">OFFICIAL - SENSITIVE</p>
        ...
        <h3 style="font-size: 11pt; margin-top: 28px;">Eligibility Character Paper</h3>
        <p style="margin-top: 28px;">for</p>
        <p style="margin-top: 13px;">${exercise.name}</p>
    ...
    Object.keys(config.APPLICATION.ELIGIBILITY_ISSUE_STATUS).forEach((k) => {

      const currentStatus = config.APPLICATION.ELIGIBILITY_ISSUE_STATUS[k];
      const statusText = k.split('_').join(' '); // i.e. REJECT_NON_DECLARATION becomes REJECT NON DECLARATION

      statusGroupARs = applicationRecords.filter(ar => ( // get applications with the status we are processing
        ar.issues && ar.issues.eligibilityIssuesStatus && ar.issues.eligibilityIssuesStatus === currentStatus
      ));
      ...
      writer.addRaw('<hr class="pb">'); // break page
      writer.addRaw(`
        <p style="font-weight: 700; font-size: 10pt; text-align: center;">OFFICIAL - SENSITIVE</p>
      `);


      if (currentStatus === config.APPLICATION.ELIGIBILITY_ISSUE_STATUS.PROCEED) {
        writer.addRaw(`
          <p style="font-weight: 700; font-size: 11pt; text-align: center; margin: 10px 0">
      ...
      statusGroupARs.forEach((applicationRecord) => {
        ...
        const comment = applicationRecord.application.quasiJudicialSatForThirtyDays || '';
        writer.addRaw(`
          <tr>
            <td><b>${lastName}</b></td>
            <td>${firstName}</td>
            <td>
              ...
              <br>
              <br>
              <b>JAC Comments (with reference to the ASC Log if appropriate):</b>
              </td>
          </tr>
        `);
      });
