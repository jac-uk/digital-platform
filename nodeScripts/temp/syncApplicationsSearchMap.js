...
/**
 * Node script to update the searchMap (_search) in all applications
...
async function updateAllApplications() {
  const commands = [];
  const applications = await getDocuments(db.collection('applications').select('personalDetails'));
  for (let i = 0, len = applications.length; i < len; ++i) {
    ...
    const hasReferenceNumber = objectHasNestedProperty(application, 'personalDetails.referenceNumber');
    ...
    if (hasReferenceNumber) {
      searchables.push(application.personalDetails.referenceNumber);
    }
